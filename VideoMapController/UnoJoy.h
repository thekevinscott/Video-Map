...
#define UNOJOY_H
	...
	typedef struct dataForController_t
	{
		...
		uint16_t leftStickX : 8;  // Each of the analog stick values can range from 0 to 255
		uint16_t leftStickY : 8;  //  0 is fully left or up
		uint16_t rightStickX : 8; //  255 is fully right or down 
		uint16_t rightStickY : 8; //  128 is centered.
